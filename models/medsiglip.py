import torch
import torch.nn as nn
import torch.nn.functional as F
from transformers import SiglipModel, SiglipProcessor, SiglipImageProcessor


class MedSigLIP(nn.Module):
    def __init__(self, backbone="google/medsiglip-448", *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)

        self.model = SiglipModel.from_pretrained(backbone) 
        self.feat_dim = 1152

    def forward_clip(self, images, text_features):
        return self.model.forward(input_ids=text_features, pixel_values=images, return_loss=False)

    def encode_text(self, text):
        return self.model.get_text_features(text.to(next(self.model.parameters()).device))

    def forward(self, images):
        return self.model.get_image_features(images)

    def from_pretrained(self, path):
        pass
